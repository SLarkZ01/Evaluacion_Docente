...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 27-05-2025 a las 17:37:15
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateSancion` (IN `p_id_docente` INT, IN `p_numero_resolucion` VARCHAR(255), IN `p_fecha_emision` DATE, IN `p_tipo_sancion` VARCHAR(255), IN `p_antecedentes` TEXT, IN `p_fundamentos` TEXT, IN `p_resolucion` TEXT, IN `p_firma_path` VARCHAR(255))   BEGIN
    ...
    SELECT * FROM sanciones WHERE id_sancion = LAST_INSERT_ID();
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `create_usuario` (IN `p_id_rol` INT, IN `p_activo` BOOLEAN, IN `p_nombre` VARCHAR(100), IN `p_apellido` VARCHAR(100), IN `p_correo` VARCHAR(150), IN `p_contrasena` VARCHAR(255), IN `p_identificacion` VARCHAR(50), IN `p_tipo_usuario` ENUM('docente','coordinador','administrador'))   BEGIN
    INSERT INTO usuarios (
        id_rol,
        activo,
        nombre,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GuardarSancion` (IN `p_id_docente` INT, IN `p_numero_resolucion` VARCHAR(50), IN `p_fecha_emision` DATE, IN `p_tipo_sancion` ENUM('leve','grave','retiro'), IN `p_antecedentes` TEXT, IN `p_fundamentos` TEXT, IN `p_resolucion` TEXT, IN `p_firma_path` VARCHAR(255), OUT `p_resultado` VARCHAR(100), OUT `p_mensaje` VARCHAR(255))   BEGIN
    DECLARE docente_existe INT;
    DECLARE resolucion_existe INT;

    -- Verificar si el docente existe
    SELECT COUNT(*) INTO docente_existe FROM docente WHERE id_docente = p_id_docente;

    IF docente_existe = 0 THEN
        SET p_resultado = 'ERROR';
        SET p_mensaje = 'El docente no existe';
    ELSE
        -- Verificar si el número de resolución ya existe
        SELECT COUNT(*) INTO resolucion_existe FROM sanciones WHERE numero_resolucion = p_numero_resolucion;

        IF resolucion_existe > 0 THEN
            SET p_resultado = 'ERROR';
            SET p_mensaje = 'El número de resolución ya está registrado';
        ELSE
            -- Insertar la sanción
            INSERT INTO sanciones (
                id_docente, 
            ...
            SET p_resultado = 'OK';
            SET p_mensaje = 'Sanción registrada correctamente';
        END IF;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerHistorialSancionesDocente` (IN `p_id_docente` INT)   BEGIN
    SELECT 
        s.id_sancion,
        s.numero_resolucion,
        s.fecha_emision,
    ...
    FROM 
        sanciones s
    JOIN 
        docente d ON s.id_docente = d.id_docente
    JOIN 
        usuarios u ON d.id_usuario = u.id_usuario
    WHERE 
        s.id_docente = p_id_docente
    ORDER BY 
        s.fecha_emision DESC;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerSancionesDocente` (IN `p_id_docente` INT)   BEGIN
    SELECT 
        ps.*,
        u.nombre AS nombre_docente,
        u.apellido AS apellido_docente
    FROM Proceso_Sancion ps
    JOIN Docente d ON ps.id_docente = d.id_docente
    JOIN Usuarios u ON d.id_usuario = u.id_usuario
    WHERE ps.id_docente = p_id_docente
    ORDER BY ps.id_proceso DESC;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerUltimaSancionDocente` (IN `p_docente_id` INT)   BEGIN
    SELECT 
        s.*,
        u.nombre AS docente_nombre,
        u.apellido AS docente_apellido,
        u.correo AS docente_email,
        u.identificacion AS docente_identificacion
    FROM 
        sanciones s
    JOIN 
        usuarios u ON s.id_docente = u.id_docente
    WHERE 
        s.id_docente = p_docente_id
    ORDER BY 
        s.created_at DESC
    LIMIT 1;
END$$
...
(11, 'ACTA-0682d9c7-36ed-11f0-aae6-48e7dabfd0bc', '2025-05-22', 'Carlos ruiz', 'sdfs', '1030405', 'INGENIERIA AMBIENTAL', 4.00, 'Aquí el decano hará sus comentarios hacia el respectivo docent', NULL, '2025-05-22 09:13:24', '2025-05-22 09:13:24'),
...
(23, 'ACTA-af52e788-38d7-11f0-aae6-48e7dabfd0bc', '2025-05-24', 'Alejo', 'xcv', '1934943', 'ELECTIVA I: OPTATIVA', 4.00, 'ya casi se sale de la carrera', NULL, '2025-05-24 19:45:01', '2025-05-24 19:45:01'),
(24, 'ACTA-413e78cc-3b08-11f0-ba0d-0a0027000009', '2025-05-27', 'Alejo', 'xcv', '1934943', 'ELECTIVA I: OPTATIVA', 4.00, 'fdghnkccdn', NULL, '2025-05-27 14:38:28', '2025-05-27 14:38:28');

--
-- Disparadores `acta_compromiso`
--
DELIMITER $$
CREATE TRIGGER `trg_prevenir_acta_duplicada` BEFORE INSERT ON `acta_compromiso` FOR EACH ROW BEGIN
    IF EXISTS (
        SELECT 1
        FROM acta_compromiso
        WHERE fecha_generacion = NEW.fecha_generacion
          AND identificacion_docente = NEW.identificacion_docente
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Ya existe un acta generada para este docente en la misma fecha.';
    END IF;
END
$$
DELIMITER ;
...
INSERT INTO `sanciones` (`id_sancion`, `id_docente`, `numero_resolucion`, `fecha_emision`, `tipo_sancion`, `antecedentes`, `fundamentos`, `resolucion`, `firma_path`, `created_at`, `updated_at`) VALUES
(2, 44, '01', '2025-05-24', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-24 23:33:31', '2025-05-24 23:33:31'),
(3, 45, '012', '2025-05-24', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-24 23:36:41', '2025-05-24 23:36:41'),
(4, 43, '011', '2025-05-24', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-24 23:37:23', '2025-05-24 23:37:23'),
(5, 43, '0112', '2025-05-24', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-24 23:39:11', '2025-05-24 23:39:11'),
(6, 44, '01122', '2025-05-24', 'grave', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-24 23:45:35', '2025-05-24 23:45:35'),
(7, 43, '011234', '2025-05-24', 'grave', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 00:00:35', '2025-05-25 00:00:35'),
(8, 43, '0112345', '2025-05-25', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 01:00:16', '2025-05-25 01:00:16'),
(9, 49, '1', '2025-05-25', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 01:47:44', '2025-05-25 01:47:44'),
(10, 46, '1223', '2025-05-25', 'grave', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 03:36:42', '2025-05-25 03:36:42'),
(11, 44, '12234', '2025-05-25', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 03:52:13', '2025-05-25 03:52:13'),
(12, 43, '1223445', '2025-05-25', 'leve', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-25 04:37:27', '2025-05-25 04:37:27'),
(13, 45, 'deewwsfr', '2025-05-27', 'grave', 'Describa aquí los antecedentes y el historial de evaluaciones del docente que justifican esta sanción...', 'Especifique aquí los reglamentos, estatutos y normativas institucionales que fundamentan esta decisión...', 'Detalle aquí la resolución específica y las medidas disciplinarias adoptadas...', NULL, '2025-05-27 14:39:05', '2025-05-27 14:39:05');

--
-- Disparadores `sanciones`
--
DELIMITER $$
CREATE TRIGGER `evitar_sancion_duplicada` BEFORE INSERT ON `sanciones` FOR EACH ROW BEGIN
  DECLARE existe INT;

  SELECT COUNT(*) INTO existe
  FROM sanciones
  WHERE id_docente = NEW.id_docente
    AND fecha_emision = NEW.fecha_emision;

  IF existe > 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Ya existe una sanción para este docente en la misma fecha.';
  END IF;
END
$$
DELIMITER ;
...
(8, 2, 1, 'Alejo', 'luisa.joaqui.j@uniautonoma.edu.co', '12345678', 'docente', 'xcv', 1934943),
(9, 1, 1, 'esteban', 'esteban@gmail.com', '12345678', 'docente', 'rodriguez', 0);
...
--
ALTER TABLE `sanciones`
  MODIFY `id_sancion` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
